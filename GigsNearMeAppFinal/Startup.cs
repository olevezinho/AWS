...
namespace GigsNearMe
{
    public class Startup
    {
        ...
        private void SetDatabaseAndConnectionString(IServiceCollection services)
        {
            ...
            // Key of a string value in Parameter Store identifying the Secrets Manager secret containing
            // the production database connection details. This parameter is created by the CDK, under the
            // /gigsnearme/ parameter root, and insulates the application from variability in the name of
            // the secret.
            const string dbsecretParameterName = "dbsecretsname";
            try
            {
                // Read the db secrets posted into Secrets Manager by the CDK. This gives us host, port,
                // userid, and password, which we format into the final connection string for SQL Server.
                // Production mode is intended to be run on an EC2 instance, so credentials and
                // region data will be inferred by the SDK clients from instance metadata.
